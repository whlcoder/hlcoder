...
import com.hlcoder.dao.PermissionMapper;
...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        if(user!=null){
            ...
            //用户的角色集合
            info.setRoles(userDao.getRolesName(loginName));
            //用户的角色对应的所有权限，如果只使用角色定义访问权限，下面的四行可以不要
            Set<Role> roleList= userDao.getRoles(loginName);
            for (Role role : roleList) {
                info.addStringPermissions(permissionMapper.getPermissionNames(role.getId()));
            }
            // 或者按下面这样添加
